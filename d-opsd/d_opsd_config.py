...
@dataclass
class dOPSDConfig(TrainingArguments):
    r"""
    ...
    ### New parameters for d-OPSD
    batch_divide: Optional[int] = field(
        default=8,
        metadata={"help": "divide batch to avoid OOM"},
    )

    debug1: bool = field(
        default=False,
        metadata={
            "help": "Whether to turn on the debug mode."
        },
    )
    ...
    passk: int = field(
        default=8,
        metadata={
            "help": "Number of reasoning trajectories to sample for each question."
        }
    )
    ...
    passk_temperature: float = field(
        default=1.0,
        metadata={
            "help": "Temperature for sampling reasoning trajectories."
        }
    )

    teacher_retain_ratio: float = field(
        default=0.25,
        metadata={
            "help": "The ratio of answer trajectories to retain from the teacher during training."
        }
    )
    ...
    fixed_teacher: bool = field(
        default=True,
        metadata={
            "help": "Whether to use a fixed teacher."
        }
    )

    top_k_loss: Optional[int] = field(
        default=20,
        metadata={
            "help": "Number of highest probability vocabulary tokens to keep for top-k-filtering (to save computation memory)."
        }
    )
    ...
    beta: float = field(
        default=1,
        metadata={
            "help": "Forward or reverse KL. 1 for reverse, and 0 for forward."
        },
    )

    jsd_token_clip: Optional[float] = field(
        default=0.05,
        metadata={
            "help": "Maximum token-wise JSD loss to prevent outliers from dominating the loss. Refer to the point-wise clipping in the paper."
        }
    )
    ...
    completion_logging_steps: Optional[int] = field(
        default=5,
        metadata={
            "help": "Number of steps between logging completions."
        }
    )
    ...
    add_ref: bool = field(
        default=False,
        metadata={
            "help": "Whether to add reference solutions to the prompt. Refer to the AR-style counterpart."
        }
    )

    teacher_max_prompt_length: Optional[int] = field(
        default=400,
        metadata={
            "help": "Maximum length of the prompt for the teacher model. This is used when `add_ref` is set to `True`, to accommodate the longer prompts that include reference solutions."
        }
    )

    diff_student_mask: Optional[bool] = field(
        default=False,
        metadata={
            "help": "Distribution selected from the teacher or student. False for selecting the teacher (default), True for student."
        }
    )

    sudoku_threshold: Optional[float] = field(
        default=0.25,
        metadata={
