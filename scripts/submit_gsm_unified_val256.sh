...
for condition in answer_prompt answer_clamp self_future; do
  include_base=false
  if [[ "$condition" == "answer_prompt" ]]; then
    include_base=true
  fi
  job_id=$(
    "$SBATCH" --parsable \
      ...
      --export="ALL,CONDITION=${condition},VALIDATION_SAMPLES=256,CHECKPOINT_STEPS=50 100 150 200 250 300,INCLUDE_BASE=${include_base},EVAL_TAG=val256-p1-seed42-v1,NUM_PROCESSES=1" \
