...
for condition in answer_prompt answer_clamp self_future; do
  job_id=$(
    "$SBATCH" --parsable \
      --partition="$PARTITION" --exclude=g55 --nodes=1 --gres=gpu:4 \
      --cpus-per-task=16 --mem=160G --time=08:00:00 \
      --job-name="dopsd-${condition}-v256" \
      --output="$ROOT/logs/%x-%j.out" --error="$ROOT/logs/%x-%j.err" \
      --export="ALL,CONDITION=${condition},VALIDATION_SAMPLES=256,CHECKPOINT_STEPS=50 100 150 200 250 300,EVAL_TAG=val256-p1-seed42-v1,NUM_PROCESSES=4" \
