...
cd "$REPO_ROOT"
for condition in group_answer_prompt group_answer_clamp group_answer_self_future; do
  run_name="${condition}-gsm-seed42-steps300-group-donor-v2"
  job_id=$(
    "$SBATCH" --parsable \
      --partition="$PARTITION" \
      --exclude=g55 \
      --job-name="dopsd-${condition}-gsm-g300" \
